...
# Data quality, missingness, imputation checks
# Source: 2_MissingCheck_FINAL.ipynb
# ============================================================

# --- SHARED SETUP ---
import os
os.makedirs("intermediary", exist_ok=True)
os.makedirs("Final/NB2", exist_ok=True)
...
# --- SECTION: Variable-Level Missingness ---

# Horizontal bar chart: % missing per variable, colour-coded by severity
def plot_variable_missingness():
    ...
    rents_1995 = (
        master[master["Year"] == 1995]
        .dropna(subset=["Total natural resources rents (% of GDP)"])
    )
    resource_countries = rents_1995[
        rents_1995["Total natural resources rents (% of GDP)"] > 5
    ]["Country Code"].unique()
    ...
    plt.tight_layout()
    plt.savefig('Final/NB2/plot1_variable_missingness.png', dpi=150, bbox_inches='tight')
    plt.show()


# --- SECTION: Country-Level Missingness ---

# Histogram of country-level missingness rates with median and mean lines
def plot_country_missingness_distribution():
    master = pd.read_csv("intermediary/master_data_wide.csv")
    if "Unnamed: 0" in master.columns:
        master.drop(columns="Unnamed: 0", inplace=True)
    ...
    plt.tight_layout()
    plt.savefig('Final/NB2/plot2_country_distribution.png', dpi=150, bbox_inches='tight')
    plt.show()


# Horizontal bar chart: all countries ranked by % missing
def plot_all_countries_ranked():
    master = pd.read_csv("intermediary/master_data_wide.csv")
    if "Unnamed: 0" in master.columns:
        master.drop(columns="Unnamed: 0", inplace=True)
    ...
    plt.tight_layout()
    plt.savefig('Final/NB2/plot3_all_countries_ranked.png', dpi=150, bbox_inches='tight')
    plt.show()


# Horizontal bar chart: variable coverage by number of countries with data
def plot_variable_coverage():
    master = pd.read_csv("intermediary/master_data_wide.csv")
    if "Unnamed: 0" in master.columns:
        master.drop(columns="Unnamed: 0", inplace=True)
    ...
    plt.tight_layout()
    plt.savefig('Final/NB2/plot4_variable_coverage.png', dpi=150, bbox_inches='tight')
    plt.show()


# --- SECTION: Missingness Diagnostics (Heatmap and Time Trend) ---

# Heatmap of missingness: top 25 problem countries x top 20 problem variables
def plot_heatmap_problem_areas():
    master = pd.read_csv("intermediary/master_data_wide.csv")
    if "Unnamed: 0" in master.columns:
        master.drop(columns="Unnamed: 0", inplace=True)
    ...
    plt.tight_layout()
    plt.savefig('Final/NB2/plot5_heatmap_problem_areas.png', dpi=150, bbox_inches='tight')
    plt.show()


# Line chart: average % missing across all variables over time
def plot_missingness_over_time():
    ...
    plt.tight_layout()
    plt.savefig('Final/NB2/plot6_missingness_over_time.png', dpi=150, bbox_inches='tight')
    plt.show()


# --- SECTION: Interactive Country Profile ---

# Interactive scatter: coverage (variables with data) vs overall % missing per country
def plot_interactive_country_profile():
    master = pd.read_csv("intermediary/master_data_wide.csv")
    if "Unnamed: 0" in master.columns:
        master.drop(columns="Unnamed: 0", inplace=True)
    ...
    fig.show()
    fig.write_html('Final/NB2/plot7_interactive_country_profile.html')


# --- SECTION: Correlation Diagnostics ---

# Full correlation matrix heatmap (lower triangle, static) plus high-correlation pair table
def plot_correlation_matrix_static():
    master = pd.read_csv("intermediary/master_data_wide.csv")
    if "Unnamed: 0" in master.columns:
        master.drop(columns="Unnamed: 0", inplace=True)
...
# --- SECTION: Interactive Correlation Matrix ---

# Interactive heatmap showing pairwise correlations (|r| > 0.5) with hover details
def plot_interactive_correlation_matrix():
    ...
    fig.show()
    fig.write_html('Final/NB2/correlation_matrix_interactive.html')

...
# =============================================================================
# CHART 01 — World choropleth: 54-country sample, coloured by cluster
# =============================================================================
print('\n=== CHART 01 ===')
...
# NR rents gradient: use 1995 values
nr_rents = (nr_nr[nr_nr['Year'] == 1995]
            .groupby('Country Code')['Production_TotalValue']
            .sum().reset_index()
            .rename(columns={'Production_TotalValue': 'NR_total'}))

map_df = cl95.merge(master[master['Year'] == 1995][['Country Code', 'Country Name']].drop_duplicates(),
                    on='Country Code', how='left')
map_df = map_df.merge(nr_rents, on='Country Code', how='left')
...
for lbl in sorted(map_df['ClusterLabels'].dropna().unique()):
    ...
    fig01.add_trace(go.Choropleth(
        ...
        z=sub['NR_total'].fillna(0),
        colorscale=[[0, color], [1, color]],
        showscale=False, showlegend=True, name=lbl,
        customdata=sub[['Country Name', 'ClusterLabels']].values,
...
fig01.update_layout(
    ...
    annotations=[dict(
        ...
        text='Source: NaturalResource.csv, clusters1995.csv. 54-country sample; cluster colours reflect resource profile typology (1995).',
        showarrow=False, font=dict(size=9, color='#888'), xanchor='center',
    )],
)
save(fig01, '01_sample__54_resource_dependent_countries_map', OUT, w=1200, h=540)


# =============================================================================
# CHART 02 — Variable correlations with ECI (Pearson, panel 1995–2019)
# =============================================================================
print('\n=== CHART 02 ===')
...
save(fig02, '02_sample__variable_correlations_with_eci', OUT, w=1100, h=600)
...
save(fig03, '03_cluster__pca_biplot_country_resource_groups', OUT, w=1100, h=680)


# =============================================================================
# CHART 04a — Cluster world map, 1995 snapshot
# =============================================================================
print('\n=== CHART 04a ===')
...
save(fig04a, '04a_cluster__world_map_1995_resource_profiles', OUT, w=1200, h=520)


# =============================================================================
# CHART 04b — Cluster world map, 2019 snapshot
# =============================================================================
print('\n=== CHART 04b ===')
...
save(fig04b, '04b_cluster__world_map_2019_resource_profiles', OUT, w=1200, h=520)


# =============================================================================
# CHART 04c — Cluster world map, aggregated (1995/1999/2005)
# =============================================================================
print('\n=== CHART 04c ===')
...
save(fig04c, '04c_cluster__world_map_agg_resource_profiles', OUT, w=1200, h=520)


# =============================================================================
# CHART 04d — 4-feature map (Oil / Gas / Coal / Minerals), k=5
# =============================================================================
print('\n=== CHART 04d ===')
...
save(fig04d, '04d_cluster__world_map_4feat_oil_gas_coal_minerals', OUT, w=1200, h=520)


# =============================================================================
# CHART 05 — Animated Rosling chart: ECI vs log(GDP pc), 1995–2019
# =============================================================================
print('\n=== CHART 05 ===')
...
save(fig05, '05_cluster__eci_vs_gdp_animated_1995_to_2019', OUT, w=1200, h=700)
...
if os.path.exists(_imp_path):
    ...
    save(fig07, '07_ml__feature_importance_consensus_three_models', OUT, w=1100, h=560)
else:
...
if os.path.exists(_tbl_path):
    ...
    save(fig08, '08_ml__standardised_coefficients_lasso_ridge_en', OUT, w=1100, h=620)
else:
...
if os.path.exists(_perf_l_path) and os.path.exists(_perf_d_path):
    ...
    save(fig09, '09_ml__train_vs_test_r2_all_models', OUT, w=1100, h=440)
else:
...
if os.path.exists(_preds_path):
    ...
    save(fig10, '10_ml__actual_vs_predicted_eci_test_set', OUT, w=1100, h=560)
else:
    print('  SKIPPED: test_predictions.csv not found')


# =============================================================================
# CHART 11 — ECI Forecast: best improvers vs worst decliners (two panels)
# =============================================================================
print('\n=== CHART 11 ===')
...
if os.path.exists(_fc_path) and os.path.exists(_rank_path):
    ...
    save(fig11, '11_ml__eci_forecast_top_improvers_2020_2030', OUT, w=1200, h=620)
else:
    print('  SKIPPED: ECI_Forecast or Country_Ranking CSV not found')


# =============================================================================
# CHART 11b — ECI Forecast heatmap, all countries (historical + projected)
# =============================================================================
print('\n=== CHART 11b ===')
...
if os.path.exists(_fc_path):
    ...
    save(fig11b, '11b_ml__eci_forecast_heatmap_all_countries', OUT,
...
# =============================================================================
# CHART 13 — ECI forecast heatmap — same as 11b but named 13
...
if os.path.exists(_fc_path):
    ...
    save(fig11b, '13_ml__eci_forecast_heatmap_historical_and_projected', OUT,
         w=1200, h=max(500, len(pivot_hm) * 12))
else:
    print('  SKIPPED: ECI_Forecast_2020_2030.csv not found')
...
# =============================================================================
# CHART 14 — ECI distribution shift: 1995 vs 2019 (CDF)
# =============================================================================
print('\n=== CHART 14 ===')
...
save(fig14, '14_reg__eci_distribution_shift_1995_vs_2019', OUT, w=1100, h=500)


# =============================================================================
# CHART 15 — ECI median trajectory by cluster (cluster names)
# =============================================================================
print('\n=== CHART 15 ===')
...
save(fig15, '15_reg__eci_mean_trajectory_by_cluster', OUT, w=1100, h=480)


# =============================================================================
# CHART 16 — Coefficients model 3a vs 3b
...
try:
    ...
    save(fig16, '16_reg__coefficients_model3a_vs_model3b', OUT, w=1100, h=700)
...
# =============================================================================
# CHART 17 — HCI × Production interaction (country means, production quartile)
# =============================================================================
print('\n=== CHART 17 ===')
...
save(fig17, '17_reg__hci_production_interaction_effect_on_eci', OUT, w=1100, h=540)


# =============================================================================
# CHART 26 — PCA Resource Loadings Heatmap (red-white-blue, PC1 on top)
# =============================================================================
print('\n=== CHART 26 ===')
...
save(fig26, '26_diag__pca_resource_loadings_heatmap', OUT, w=1300, h=420)


# =============================================================================
# CHART 27 — Random Forest Feature Importance
# =============================================================================
print('\n=== CHART 27 ===')
...
if os.path.exists(_imp_path):
    ...
    if 'Importance' in imp27.columns and 'Feature' in imp27.columns:
        ...
        save(fig27, '27_ml__random_forest_feature_importance', OUT, w=1100, h=540)
    else:
...
# =============================================================================
# CHART 28 — VIF (Variance Inflation Factors)
# =============================================================================
print('\n=== CHART 28 ===')
...
if os.path.exists(_vif_path):
    ...
    if feat_col and vif_col:
        ...
        save(fig28, '28_diag__vif_multicollinearity', OUT, w=1100, h=500)
    else:
    ...
    try:
        ...
        save(fig28, '28_diag__vif_multicollinearity', OUT, w=1100, h=440)
    except ImportError:
...
# =============================================================================
# CHART 29 — Bootstrap R² stability (ML models)
# =============================================================================
print('\n=== CHART 29 ===')
...
if os.path.exists(_boot_metrics):
    ...
    if model_cols:
        ...
        save(fig29, '29_diag__bootstrap_r2_stability', OUT, w=1100, h=500)
    else:
...
save(fig30, '30_reg__regression_variable_correlation_heatmap', OUT, w=1000, h=700)
...
if os.path.exists(_preds_path):
    ...
    save(fig31, '31_diag__ml_prediction_intervals', OUT, w=1200, h=500)
else:
    print('  SKIPPED: test_predictions.csv not found')


# =============================================================================
# CHART 32 — Country Data Coverage (scatter: vars with data vs % missing)
# =============================================================================
print('\n=== CHART 32 ===')
...
save(fig32, '32_diag__country_data_coverage_scatter', OUT, w=1100, h=520)


# =============================================================================
# CHART 33 — Full variable correlation matrix (all features in Master.csv)
# =============================================================================
print('\n=== CHART 33 ===')
...
save(fig33, '33_diag__full_variable_correlation_matrix', OUT, w=w33, h=h33)


# =============================================================================
# CHART 34 — HRV Growth Diagnostics (placeholder / best-effort)
# =============================================================================
print('\n=== CHART 34 ===')
...
if _hrv_path:
    ...
    if year_col and num_cols:
        ...
        save(fig34, '34_diag__hrv_growth_diagnostics', OUT, w=1100, h=480)
    else:
    ...
    save(fig34, '34_diag__hrv_growth_diagnostics_placeholder', OUT, w=900, h=300)

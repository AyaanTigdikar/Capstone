...
# ---------------------------------------------------------------------------
# Paths  (all relative to the project root: CLEAN/)
# ---------------------------------------------------------------------------
_INTR  = 'intermediary'
_NB5   = 'Graphics/NB5'          # NB5 saves here (not Final/NB5)
_VIZ   = 'Visualisation'         # viz output root
...
_clusters_csv_candidates = [
    os.path.join(_INTR, 'clusters_k4_1995.csv'),   # NB4 actual output
    os.path.join(_INTR, 'clusters1995.csv'),         # legacy alias
]
_clusters_csv = next((p for p in _clusters_csv_candidates if os.path.exists(p)), '')
...
def load_clusters(year: str = '1995') -> pd.DataFrame:
    """
    Load cluster assignments for a given year/variant.
    NB4 saves as clusters_k4_<label>.csv; viz scripts historically
    expected clusters<label>.csv.  Try both.
    """
    candidates = [
        os.path.join(_INTR, f'clusters_k4_{year}.csv'),   # NB4 actual output
        os.path.join(_INTR, f'clusters{year}.csv'),        # legacy alias
    ]
    path = next((p for p in candidates if os.path.exists(p)), None)
    if path is None:
        raise FileNotFoundError(
            f"No cluster CSV found for year={year}. "
            f"Looked for: {candidates}. Run NB4 first."
        )
    df = pd.read_csv(path)

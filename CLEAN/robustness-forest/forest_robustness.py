...
# ─────────────────────────────────────────────────────────────────────────────
# 3.  Regression helper: clustered SE by country — mirrors NB6 spec
# ─────────────────────────────────────────────────────────────────────────────
def run_model3(df, rent_col, label):
    """
    OLS with SE clustered by country (mirrors Model 3a in NB6).
    Returns a tidy coefficient DataFrame.
    """
    req = [ECI_COL, rent_col, 'Country Code'] + CORE_VARS
    df = df.copy().dropna(subset=req)
    ...
    df['log_Prod'] = np.log1p(df['Total_Production_Value_Per_Capita'].clip(lower=0))

    # Mean-centred interactions
    df['log_HCI_x_Prod']  = (df['log_HCI']  - df['log_HCI'].mean())  * \
                             (df['log_Prod'] - df['log_Prod'].mean())
    df['log_GFCF_x_Prod'] = (df['log_GFCF'] - df['log_GFCF'].mean()) * \
                             (df['log_Prod'] - df['log_Prod'].mean())
    ...
    res = pd.DataFrame({
        ...
        'Coef':      np.asarray(fit.params).flatten(),
        'SE':        np.asarray(fit.bse).flatten(),
        'p':         np.asarray(fit.pvalues).flatten(),

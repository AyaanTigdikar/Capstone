"""
Builds 5_ML_FINAL.ipynb (unified) from the code of both
5_ML_FINAL and 5_ML_TEST, with the following design:

Structure
─────────
0  Setup & Data Loading
1  Feature Engineering   (+ 3 new features from TEST)
2  Train / Test Split    (TEST's temporal split + train-only scaler)
3  Model Training        (fit_all_models helper from TEST)
4  Diagnostics           (VIF on X_train; train+test R² from TEST; coef importance from FINAL)
5  Static Charts         (VIF, 3-panel coefs, model agreement, RF importance from FINAL;
                          OOS R², pred vs actual, SHAP, prediction intervals from TEST)
6  Interactive Charts    (Plotly with FINAL's STYLE/PALETTE/save_chart)
7  Summary Table         (FINAL's heatmap table)
8  Forecast 2020-2030    (TEST's full section)
9  Summary print block

Key fixes
─────────
- Scaler fit on TRAIN only (fixes TEST's leakage issue that was already fixed,
  and fixes FINAL's leakage issue)
- Target: raw ECI (not log-shifted) — consistent with TEST
- All output → Graphics/NB5/
"""

import json
from pathlib import Path

OUT_NB = Path("/Users/leoss/Desktop/GitHub/Capstone/FINAL CODE RECAP/5_ML_FINAL.ipynb")
...
# ─────────────────────────────────────────────────────────────────────────────
cells.append(md("## 0. Setup and Data Loading"))
cells.append(code("""\
import os
import warnings
import numpy as np
...
- **`Resource_HHI`** — Herfindahl index of NR rent concentration (oil/gas/mineral shares)
...
rents_cols  = ['Oil rents (% of GDP)', 'Natural gas rents (% of GDP)', 'Mineral rents (% of GDP)']
...
# ── Log transforms (right-skewed variables only) ──────────────────────────────
log_cols = [
    'Human capital index',
    'Total_Production_Value',
    'Gross fixed capital formation, all, Constant prices, Percent of GDP',
...
def fit_all_models(X_tr, y_tr, tscv, label=''):
    ...
    if HAS_XGB:
        ...
        xgb_model.fit(X_tr[:-n_val], y_tr[:-n_val],
...
tscv_full   = TimeSeriesSplit(n_splits=5, gap=1)

models_full_level = fit_all_models(X_full, y_full_level, tscv_full, label='ECI (full)')
models_full_delta = fit_all_models(X_full, y_full_delta, tscv_full, label='ΔECI (full)')
...
for cc, cdf in df.groupby('Country Code'):
    ...
    for _, frow in future_sub.iterrows():
        ...
        for name, model in models_full_level.items():
            ...
            row_data['HCI_x_ProductionValue']       = row_data['Human capital index'] * row_data['Total_Production_Value']
            row_data['RuleOfLaw_x_ProductionValue'] = row_data['Rule of law index']   * row_data['Total_Production_Value']
...
plt.tight_layout()
plt.savefig(os.path.join(OUT, 'Forecast_CountryRanking.png'), dpi=300, bbox_inches='tight', facecolor='white')
plt.savefig(os.path.join(OUT, 'Forecast_CountryRanking.pdf'), bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig(os.path.join(OUT, 'Forecast_TopImprovers_Trajectory.png'), dpi=300, bbox_inches='tight', facecolor='white')
plt.savefig(os.path.join(OUT, 'Forecast_TopImprovers_Trajectory.pdf'), bbox_inches='tight')
plt.show()
